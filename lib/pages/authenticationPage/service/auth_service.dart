...
class AuthService {
            ...
            if (isItAPatient == true) {
              ...
              if (addPatientResponse == true) {
                registerResponseModel = const RegisterResponseModel(
                    message: 'Registration Successful!', status: true);
              } else {
                registerResponseModel = const RegisterResponseModel(
  ...
  Future<LogoutResponseModel?> logoutUser() async {
    ...
    try {
      var networkConnectionResponse = await connection.isInternetEnabled();
      if (networkConnectionResponse == true) {
        var logoutAuthUserResponse = await logOut();
        if (logoutAuthUserResponse!.toLowerCase() == 'success') {
          logoutResponseModel = const LogoutResponseModel(
              message: 'Logout successful!',
              status: true,
            );
        } else {
          logoutResponseModel =
              LogoutResponseModel(message: logoutAuthUserResponse, status: false);
        }
      } else {
        logoutResponseModel = const LogoutResponseModel(
            message: 'Check internet connection', status: false);
